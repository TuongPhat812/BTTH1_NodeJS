const mongoClient = require('mongodb').MongoClient;

mongoClient.connect('mongodb://127.0.0.1:27017/qlbh', function (err, client) {
    //neu ket noi khong thanh cong thi in ra loi
    if (err) throw err;
    //neu thanh cong thi log ra thong bao

    console.log('Ket noi thanh cong');

    var db = client.db('quanlybanhoa');
    //tao collection
    //chi tiet don hang
    db.createCollection('hoa', function (err, res) {
        if (err) throw err;
        console.log('Tao thanh cong collection');
    });

    //don hang
    db.createCollection('khachhang', function (err, res) {
        if (err) throw err;
        console.log('Tao thanh cong collection');
    });

    //danh muc category
    db.createCollection('loaihoa', function (err, res) {
        if (err) throw err;
        console.log('Tao thanh cong collection');
    });



    var hoa = db.collection('hoa');
    var khachhang = db.collection('khachhang');
    var loaihoa = db.collection('loaihoa');


    var insertCall = function (err, res) {
        //neu xay ra loi
        if (err) throw err;
        //neu khong co loi
        console.log('Them thanh cong');
        // console.log(res)
    };

    //insert data cho chi tiet hoa don
    var data = [{
            mahoa: "1",
            maloai: "Hoa-Cuc",
            tenhoa: "Đón xuân",
            giaban: "50000",
            hinh: "cuc_9.jpg",
            mota: "Hoa cúc các màu: trắng, vàng, cam\r\n"
        },
        {
            mahoa: "2",
            maloai: "Hoa-Cuc",
            tenhoa: "Hồn nhiên",
            giaban: "60000",
            hinh: "cuc_2.jpg",
            mota: "Hoa cúc vàng, cam. lá măng\r\n"
        },
        {
            mahoa: "3",
            maloai: "Hoa-Cuc",
            tenhoa: "Tím thuỷ chung",
            giaban: "45000",
            hinh: "cuc_3.jpg",
            mota: "Hoa cúc tím\r\n"
        }, {
            mahoa: "4",
            maloai: "Hoa-Cuc",
            tenhoa: "Nét duyên tím hoa cà",
            giaban: "40000",
            hinh: "cuc_4.jpg",
            mota: "Hoa cúc màu tím nhạt\r\n"
        }, {
            mahoa: "5",
            maloai: "Hoa-Cuc",
            tenhoa: "Cùng khoe sắc",
            giaban: "70000",
            hinh: "cuc_5.jpg",
            mota: "Hoa cúc các màu: trắng, vàng, cam\r\n"
        }, {
            mahoa: "6",
            maloai: "Hoa-Cuc",
            tenhoa: "Trắng thơ ngây",
            giaban: "65000",
            hinh: "cuc_6.jpg",
            mota: "Hoa cúc trắng\r\n"
        }, {
            mahoa: "7",
            maloai: "Hoa-Cuoi",
            tenhoa: "Dây tơ hồng",
            giaban: "250000",
            hinh: "cuoi_1.jpg",
            mota: "Hoa hồng màu hồng đậm, hoa hồng xanh, các loại lá măng\r\n"
        }, {
            mahoa: "8",
            maloai: "Hoa-Cuoi",
            tenhoa: "Cầu thuỷ tinh",
            giaban: "220000",
            hinh: "cuoi_2.jpg",
            mota: "Hoa hồng và hoa thuỷ tiên trắng\r\n"
        }, {
            mahoa: "9",
            maloai: "Hoa-Cuoi",
            tenhoa: "Duyên thầm",
            giaban: "260000",
            hinh: "cuoi_3.jpg",
            mota: "Hoa cúc trắng, baby, lá măng\r\n"
        }, {
            mahoa: "10",
            maloai: "Hoa-Cuoi",
            tenhoa: "Đâm chồi nảy lộc",
            giaban: "180000",
            hinh: "cuoi_4.jpg",
            mota: "Hoa hồng trắng và các loại lá măng\r\n"
        }, {
            mahoa: "11",
            maloai: "Hoa-Cuoi",
            tenhoa: "Hoà quyện",
            giaban: "270000",
            hinh: "cuoi_5.jpg",
            mota: "Hoa hồng trắng, lá thuỷ tùng\r\n"
        }, {
            mahoa: "12",
            maloai: "Hoa-Cuoi",
            tenhoa: "Nồng nàn",
            giaban: "210000",
            hinh: "cuoi_6.jpg",
            mota: "Hoa hồng đỏ, lá thuỷ tùng, lá măng\r\n"
        }, {
            mahoa: "13",
            maloai: "Hoa-Hong",
            tenhoa: "Together",
            giaban: "120000",
            hinh: "hong_1.jpg",
            mota: "Hồng xác pháo, cúc tím\r\n"
        }, {
            mahoa: "14",
            maloai: "Hoa-Hong",
            tenhoa: "Long trip",
            giaban: "85000",
            hinh: "hong_2.jpg",
            mota: "Hoa hồng đỏ, lá kim thuỷ tùng\r\n"
        }, {
            mahoa: "15",
            maloai: "Hoa-Hong",
            tenhoa: "Beautiful life",
            giaban: "100000",
            hinh: "hong_3.jpg",
            mota: "Hoa hồng đỏ, lá măng, lá đệm\r\n"
        }, {
            mahoa: "16",
            maloai: "Hoa-Hong",
            tenhoa: "Morning Sun",
            giaban: "75000",
            hinh: "hong_4.jpg",
            mota: "Hoa hồng vàng\r\n"
        }, {
            mahoa: "17",
            maloai: "Hoa-Hong",
            tenhoa: "Pretty Bloom",
            giaban: "65000",
            hinh: "hong_5.jpg",
            mota: "Hoa hồng trắng và lá thông\r\n"
        }, {
            mahoa: "18",
            maloai: "Hoa-Hong",
            tenhoa: "Red Rose",
            giaban: "45000",
            hinh: "hong_7.jpg",
            mota: "Hoa hồng đỏ và lá măng\r\n"
        }, {
            mahoa: "19",
            maloai: "Hoa-Xuan",
            tenhoa: "Vấn vương",
            giaban: "150000",
            hinh: "xuan_1.jpg",
            mota: "Hoa hồng đỏ, hồng đậm, cúc đỏ, các loại lá\r\n"
        }, {
            mahoa: "20",
            maloai: "Hoa-Xuan",
            tenhoa: "Nắng nhẹ nhàng",
            giaban: "50000",
            hinh: "xuan_2.jpg",
            mota: "Hoa cúc xanh, hoa lys vàng, lá đệm\r\n"
        }, {
            mahoa: "21",
            maloai: "Hoa-Xuan",
            tenhoa: "Thanh tao",
            giaban: "120000",
            hinh: "xuan_3.jpg",
            mota: "Hoa thủy tiên, cúa trắng, cúc dại tím, các loại lá đệm\r\n"
        }, {
            mahoa: "22",
            maloai: "Hoa-Xuan",
            tenhoa: "Tinh khiết",
            giaban: "110000",
            hinh: "xuan_4.jpg",
            mota: "Hồng trắng và salem\r\n"
        }, {
            mahoa: "23",
            maloai: "Hoa-Xuan",
            tenhoa: "Mùa xuân chín",
            giaban: "150000",
            hinh: "xuan_5.jpg",
            mota: "Hồmg cam, cúc xanh, thuỷ tiên và các loại lá\r\n"
        }, {
            mahoa: "24",
            maloai: "Hoa-Xuan",
            tenhoa: "Rực rỡ nắng vàng",
            giaban: "75000",
            hinh: "xuan_6.jpg",
            mota: "Hồng vàng và cúc vàng\r\n"
        }, {
            mahoa: "25",
            maloai: "Hoa-Hong",
            tenhoa: "Love Candy",
            giaban: "95000",
            hinh: "hong_13.jpg",
            mota: "Hoa hồng trắng tinh khôi xen với hoa hồng màu hồng nhạt, điểm thêm baby trắng và lá măng\r\n"
        }, {
            mahoa: "26",
            maloai: "Hoa-Cuoi",
            tenhoa: "Happy Wedding",
            giaban: "210000",
            hinh: "cuoi_9.jpg",
            mota: "Hoa hồng môn và các loại lá"
        }, {
            mahoa: "27",
            maloai: "Hoa-Cuc",
            tenhoa: "Cúc nhiệt đới",
            giaban: "150000",
            hinh: "cuc_15.jpg",
            mota: "Cúc vàng - hồng cam - lá măng"
        }
    ]
    hoa.insertMany(data, insertCall);

    
    //insert data cho danh muc
    data = [
        {maloai:"Hoa-Cuc",tenloai:"Hoa cúc"},
        {maloai:"Hoa-Cuoi",tenloai:"Hoa cưới"},
        {maloai:"Hoa-Hong",tenloai:"Hoa hồng"},
        {maloai:"Hoa-Xuan",tenloai:"Hoa xuân"}
        ]
    loaihoa.insertMany(data, insertCall);

    //insert cho don hang
    data = [{
        ten_dn: "phuong",
        mat_khau: "phuong",
        mat_khau1: "phuong",
        ho_ten: "Hoàng Phương",
        email: "phuong@gmail.com",
        dia_chi: "uit",
        so_dt: "0125367159",
        Submit: "Đăng ký"
      }]
    khachhang.insertMany(data, insertCall);

    client.close();
    console.log('Dong ket noi thanh cong');

});